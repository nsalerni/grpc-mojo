...
the `h2` protocol. The blocking server can require a client certificate that
chains to a configured client CA before it accepts HTTP/2 requests.
...
reference nesting-depth limit. `PollingServer` does not yet authenticate
client certificates, and neither server selects certificates by SNI. The
