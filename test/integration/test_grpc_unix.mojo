# gRPC over Unix domain sockets, including safe handling of stale paths.
...
def wait_for_socket(path: String) raises:
    for _ in range(100):
        try:
            ...
            sleep(0.01)
    raise Error("Unix gRPC server did not create its socket")


def echo(req: EchoRequest, mut ctx: ServerContext) raises -> EchoResponse:
    var response = EchoResponse()
    response.message = String("unix: ") + req.message
    return response^


def test_unary_over_unix() raises:
    ...
    if pid == 0:
        try:
            var server = Server.unix(path, remove_existing=True)
    ...
    try:
        ...
        assert_equal(response.message, "unix: hello")
...
def test_stale_path_requires_opt_in() raises:
    ...
    if not child_done:
        stop_child(pid, path.copy())
        raise Error("Server.unix unexpectedly accepted a stale path")
    remove_socket(path^)
    assert_equal(Int(status), 0)


def test_empty_authority_rejected() raises:
    var raised = False
    try:
        _ = GrpcChannel.connect_unix("/path/that/does/not/exist", authority="")
    except e:
        raised = "authority cannot be empty" in String(e)
    assert_true(raised, "connect_unix must reject an empty authority")


def main() raises:
    test_empty_authority_rejected()
    test_unary_over_unix()
    test_stale_path_requires_opt_in()
    print("test_grpc_unix: ok")
