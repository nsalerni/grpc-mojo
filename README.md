...
**[📋 Compliance report](https://nsalerni.github.io/grpc-mojo/docs/COMPLIANCE.html)** ([Markdown](docs/COMPLIANCE.md)) — 56 differential checks against protobuf, python-hpack, hyper-h2, h2spec, CPython sockets, and grpcio; regenerated by every CI run.
...
- **Official gRPC interop cases**: 12 canonical cases × both directions
  vs `grpcio` — 24/24 (`pixi run interop-official`)
...
plaintext h2c only (TLS needs a Mojo TLS binding), sequential connection
handling and recv-driven bidi (Mojo 1.0 has no public threads/async),
no compression codecs yet (zlib binding pending).
...
| `mojo-h2` | [`packages/mojo-http2`](packages/mojo-http2) | HTTP/2 frames, streams, connections | `hpack`, `net` |
...
# standalone checkout only: fetch the mojo-net/protomojo/mojo-http2
...
(currently **56/56 checks passing**). Each package repo also runs its own
