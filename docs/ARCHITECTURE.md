...
h2c, TLS, or Unix connections on one thread; handlers still run
serially. Streaming handlers switch that connection to blocking I/O for
the duration of the call. Unix listeners are plaintext only.
`request_stop()` drains via GOAWAY on that same thread; there is no
cross-thread stop.
